...
//        }
    ...
    public static String formatSequence(String dateFormat, byte btValue[], int iOffset, int iLength, int seqType, String pattern, double timezone) {
        if (seqType == 16) {
            int countServices = iLength / seqType;
            String strSeq = "";
            for (int i = 0; i < countServices; i++) {
                String serType = getStrByte(btValue[iOffset + i * 16 + 4]);
                String serTime = formatHuaWeiTimeStamp(dateFormat, btValue, iOffset + i * 16 + 7, 9, 0, timezone);
    ...
    public static String formatHuaWeiTimeStamp(String format, byte btValue[], int iOffset, int iLength, int iUtcOffset, double timezone) {
        if (btValue.length < iOffset + iLength || iLength < 9) {
            return "";
        }
        Date dtReturn = null;
